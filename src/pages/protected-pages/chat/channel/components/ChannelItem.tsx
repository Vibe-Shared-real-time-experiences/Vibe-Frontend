...
const ChannelItem = ({ channel, activeChannelId, onChangeChannel }: { channel: ChannelResponse; activeChannelId: string | null; onChangeChannel: (channelId: string) => void }) => {

    return (
        <div
            onClick={() => onChangeChannel(channel.id)}
            key={channel.id}
            className={`group flex items-center justify-between px-2 py-[6px] rounded transition ${activeChannelId === channel.id
        ...
        >
            ...
            <div className={`flex gap-1 flex-shrink-0 opacity-0 group-hover:opacity-100 transition ${activeChannelId === channel.id ? 'opacity-100' : ''}`}>
